...
def detect_anomalies_by_brand(brand_data):
    ...
    # Уникальные даты для уменьшения числа итераций
    unique_dates = brand_data['researchdate'].unique()

    for current_date in unique_dates:
        # Данные за текущий день
        today_rows = brand_data[brand_data['researchdate'] == current_date]
        ...
        # Аномалии: строки сегодняшнего дня с долей выше порога
        anomalous_rows = today_rows[today_rows['share'] > threshold]

        for _, row in anomalous_rows.iterrows():
            anomalies.append((row['SubjectID'], current_date))
            anomaly_reasons.append({
                ...
                'mad': mad,
                'threshold': threshold,
                'reason': f"share={row['share']:.4f} > threshold={threshold:.4f}"
            })
